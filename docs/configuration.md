...
| `PROM_SHIM_CLICKHOUSE_MAX_QUERY_SIZE_BYTES` | `0` | Optional per-query `max_query_size` setting; `0` leaves it unset. |
...
| `PROM_SHIM_RECORDING_RULE_FILES` | empty | Comma-separated rendered Prometheus rule YAML files or glob patterns to load when `PROM_SHIM_RECORDING_RULE_MODE=virtual`. Alerting rules are ignored; conflicting recording-rule names are rejected unless selector matchers disambiguate by static rule/group labels. |
