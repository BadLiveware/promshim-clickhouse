...
| `max_query_size` | Applied only when `PROM_SHIM_CLICKHOUSE_MAX_QUERY_SIZE_BYTES>0`. | Query/session. | Core ClickHouse setting. | Explain reason `safety_query_size_cap`; default skip reason `not_configured`. |
