...
Build a practical migration path from the current Tradera `monitoring-v2` Prometheus + Thanos stack to an **OpenTelemetry + ClickHouse** stack, with a single HA-first architecture and environment-specific capacity tuning.
...
- `~/code/tradera/helm-charts/charts/monitoring/` (published monitoring-v2 Helm chart)
...
- `~/code/tradera/tradera-iac/setup/dev-dynamic/apps/monitoring-v2/k8s/values.yaml` for environment shape and operational override values.
- `~/code/tradera/tradera-iac/kubernetes/base/tradera-applications/monitoring-v2/prod/values.yaml` for production defaults and alert routing conventions.
