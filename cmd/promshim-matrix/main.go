...
func main() {
	jsonOut := flag.String("json-out", filepath.FromSlash("harness/artifacts/matrices/native-compliance-matrix.json"), "JSON output path")
	mdOut := flag.String("md-out", filepath.FromSlash("harness/artifacts/matrices/native-compliance-matrix.md"), "Markdown output path")
	flag.Parse()
...
func buildRow(spec featureSpec) (matrixRow, error) {
	...
	status := compliance.ClassifyNativeStatus(snapshot, spec.Query)
...
func writeMarkdown(path string, doc matrixDocument) error {
	...
	for _, kind := range []string{"selector", "modifier", "aggregation", "binary", "vector_matching", "function"} {
		...
		b.WriteString("| Name | Native | Bucket | Canonical query | Notes |\n")
		b.WriteString("|---|---|---|---|---|\n")
		for _, row := range kindRows {
			_, _ = fmt.Fprintf(&b, "| `%s` | `%s` | `%s` | `%s` | %s |\n", row.Name, row.NativeStatus, row.ImplementationBucket, escapeTable(row.Query), escapeTable(strings.Join(row.Notes, "; ")))
		}
		b.WriteString("\n")
	}
...
func summarizeBuckets(rows []matrixRow, status string) []bucketSummary {
	...
	for _, row := range rows {
		if row.NativeStatus != status || row.ImplementationBucket == "" {
			continue
		}
