SELECT tags AS tags, timestamp AS timestamp, value AS value FROM (SELECT tags AS tags, timestamp AS timestamp, value AS value FROM (SELECT CAST([], 'Array(Tuple(String, String))') AS grouping_tags, tags AS tags, timestamp AS timestamp, value AS value FROM (SELECT tags AS tags, timestamp AS timestamp, value AS value FROM (
    ...
    ) AS series ON d.id = series.id WHERE d.timestamp >= fromUnixTimestamp64Milli({required_start_ms:Int64}) AND d.timestamp <= fromUnixTimestamp64Milli({required_end_ms:Int64}) AND d.id IN (SELECT id FROM (
        SELECT DISTINCT src.id, arrayConcat([tuple('__name__', src.metric_name)], arrayMap((k, v) -> tuple(k, v), mapKeys(src.tags), mapValues(src.tags))) AS tags FROM timeSeriesTags(`observability`.`prometheus`) AS src WHERE src.metric_name = {instant_matcher_0_value:String} AND src.max_time >= fromUnixTimestamp64Milli({required_start_ms:Int64}) AND src.min_time <= fromUnixTimestamp64Milli({required_end_ms:Int64})
    )) GROUP BY d.id, series.tags HAVING NOT isNaN(value) ORDER BY tags
))) ORDER BY grouping_tags, isNaN(value), value DESC, tags LIMIT 3 BY grouping_tags) ORDER BY tags
