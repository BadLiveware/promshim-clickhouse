SELECT tags AS tags, arraySort(item -> item.1, groupArray((timestamp, value))) AS time_series FROM (
    SELECT arraySort(tag -> tag.1, arrayFilter(tag -> NOT has(['__name__', 'instance'], tag.1), tags)) AS tags, timestamp AS timestamp, if(countIf(isNaN(value)) > 0, CAST(NULL, 'Nullable(Float64)'), sum(value)) AS value FROM (
