...
func TestShouldServeDenseRateRollupRejectsBeforeCoverageProbe(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if metricName, got := tt.service.shouldServeDenseRateRollup(context.Background(), tt.query, now, now, tt.step); got || metricName != "" {
				t.Fatalf("shouldServeDenseRateRollup() = %q, true; want false", metricName)
			}
		})
	}
}
