SELECT final_tags AS tags, arraySort(item -> item.1, groupArray((timestamp, value))) AS time_series FROM (SELECT arrayFilter(tag -> tag.1 != '__name__', tags) AS final_tags, eval_ts AS timestamp, if(length(window_values) < 2 OR arrayExists(v -> isNaN(v), window_values), nan, tupleElement(arrayReduce('simpleLinearRegression', arrayMap(ts -> (toFloat64(toUnixTimestamp64Milli(ts)) - (toFloat64(toUnixTimestamp64Milli(eval_ts)))) / 1000.0, window_timestamps), window_values), 1)) AS value FROM (
    SELECT source.tags AS tags, grid.eval_ts AS eval_ts, arrayFilter(point -> tupleElement(point, 1) <= grid.eval_ts - toIntervalMillisecond(0) AND tupleElement(point, 1) >= grid.eval_ts - toIntervalMillisecond(600000), source.time_series) AS window_series, arrayMap(point -> tupleElement(point, 1), window_series) AS window_timestamps, arrayMap(point -> ifNull(toFloat64(tupleElement(point, 2)), nan), window_series) AS window_values FROM (SELECT arrayJoin(arrayMap(ts_ms -> fromUnixTimestamp64Milli(ts_ms), range(1700000000000, 1700000300000 + 1, 60000))) AS eval_ts) AS grid CROSS JOIN (
