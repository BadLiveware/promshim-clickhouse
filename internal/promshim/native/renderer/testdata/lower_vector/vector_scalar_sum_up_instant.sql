SELECT CAST([], 'Array(Tuple(String, String))') AS tags, fromUnixTimestamp64Milli({evaluation_ms:Int64}) AS timestamp, if(count() = 1, any(value), nan) AS value FROM (
    SELECT CAST([], 'Array(Tuple(String, String))') AS tags, fromUnixTimestamp64Milli(1700000000000) AS timestamp, if(countIf(isNaN(value)) > 0, CAST(NULL, 'Nullable(Float64)'), sum(value)) AS value FROM (
        SELECT series.tags AS tags, max(d.timestamp) AS timestamp, argMax(d.value, d.timestamp) AS value FROM timeSeriesData(`observability`.`prometheus`) AS d INNER JOIN (
            ...
            SELECT DISTINCT src.id FROM timeSeriesTags(`observability`.`prometheus`) AS src WHERE src.metric_name = {scalar_child_instant_matcher_0_value:String} AND src.max_time >= fromUnixTimestamp64Milli({scalar_child_required_start_ms:Int64}) AND src.min_time <= fromUnixTimestamp64Milli({scalar_child_required_end_ms:Int64})
