SELECT tags AS tags, arraySort(item -> item.1, groupArray((timestamp, value))) AS time_series FROM (
    SELECT arrayFilter(tag -> has(['job'], tag.1), tags) AS tags, timestamp AS timestamp, if(countIf(NOT isNaN(value)) = 0, CAST(NULL, 'Nullable(Float64)'), maxIf(value, NOT isNaN(value))) AS value FROM (
