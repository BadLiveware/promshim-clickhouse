...
| Production deployments may be single-node, replicated, or distributed. | `advisory-for-performance` | Route and storage recommendations are topology-dependent; correctness must not rely on one topology. For replicated TimeSeries, use `ON CLUSTER` distributed DDL or a `Replicated` database/operator metadata path so every replica sees the same TimeSeries and inner-table UUIDs; see `docs/clickhouse-timeseries-deployment-tuning.md`. | Query-log `is_initial_query`, `initial_query_id`, distributed EXPLAIN output, per-replica `system.replicas`. |
