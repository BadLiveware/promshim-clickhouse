...
`should_fail: true` marker from entries whose queries are now accepted
by the reference Prometheus. The queries themselves are preserved, so
the tester runs them normally and compares refAPI vs testAPI.
...
def main() -> int:
    ...
    expected = len(QUERIES_NOW_ACCEPTED)
    if len(dropped) != expected:
        print(
            f"warning: expected to drop {expected} markers, dropped {len(dropped)}. "
            "Upstream corpus may have changed — review QUERIES_NOW_ACCEPTED.",
            file=sys.stderr,
        )
        return 1
    return 0
